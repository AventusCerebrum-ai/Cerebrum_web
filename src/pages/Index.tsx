...
const Index: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen dark:from-[#34205e]/50 dark:via-background/60 dark:to-background transition-colors overflow-x-hidden">
      ...
      {/* Modernized Digital Triage Section */}
      <section className="relative z-1 min-h-[60vh] flex items-center justify-center px-2 md:px-0 py-16 animate-fade-in">
        <div className="backdrop-blur-lg bg-white/10 dark:bg-[#1a2233]/30 border border-cyan-400/10 rounded-3xl shadow-2xl p-8 md:p-14 flex flex-col items-center max-w-3xl w-full mx-auto">
          <h3 className="font-bold text-2xl md:text-3xl mb-4 text-cyan-300 text-center drop-shadow">
            How CerebrumAI Modernizes Digital Triage
          </h3>
          <p className="text-lg md:text-xl text-cyan-100 text-center mb-8 max-w-2xl">
            CerebrumAI is designed to modernize digital triage through an intelligent, <span className="text-cyan-300 font-semibold">multimodal approach</span>.<br />
            By analyzing a combination of <span className="text-cyan-200">textual descriptions</span>, <span className="text-cyan-200">medical images</span>, and <span className="text-cyan-200">behavioral biometrics</span>, it delivers context-aware assessments that go beyond conventional symptom checkers—enabling safer and more actionable care journeys.
          </p>
          {/* Animated or styled image */}
          <div className="mb-8">
            {/* Replace with a Lottie animation or SVG if you have one */}
            <img
              src="/Dna.jpeg"
              alt="AI Digital Triage"
              className="rounded-2xl shadow-xl border-2 border-cyan-400/20 w-full max-w-xs mx-auto animate-pulse"
              loading="lazy"
              draggable={false}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-8">
            <FeatureCard
              icon={<Eye size={32} className="text-cyan-300" />}
              title="Multimodal Input"
              desc="Seamless capture of text, images, and behavioral data for a holistic triage."
            />
            <FeatureCard
              icon={<UserCheck size={32} className="text-cyan-300" />}
              title="Personalized AI"
              desc="AI-driven analysis delivers tailored, actionable recommendations."
            />
            <FeatureCard
              icon={<Mail size={32} className="text-cyan-300" />}
              title="Secure Doctor Access"
              desc="Connect safely with professionals only when advanced intervention is needed."
            />
          </div>
          {/* Call to Action */}
          <button
            onClick={() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' })}
            className="mt-2 px-8 py-3 bg-cyan-500 text-white rounded-full font-semibold shadow hover:bg-cyan-400 transition"
          >
            See CerebrumAI in Action
          </button>
        </div>
      </section>
...
// Place this FeatureCard component above or in the same file:
const FeatureCard = ({ icon, title, desc }) => (
  <div className="bg-cyan-900/30 rounded-xl p-5 flex flex-col items-center shadow border border-cyan-400/10">
    <div className="mb-2">{icon}</div>
    <div className="font-bold text-cyan-200 mb-1">{title}</div>
    <div className="text-cyan-100 text-sm text-center">{desc}</div>
  </div>
);
